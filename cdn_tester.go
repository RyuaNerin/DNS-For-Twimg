...
func (ct *CDNTester) testHTTPTask(w *sync.WaitGroup, host ConfigHost, cdn *CdnStatus) {
	...
	client := http.Client{
		...
		Transport: &http.Transport{
			Dial: func(network, addr string) (net.Conn, error) {
				return net.Dial(network, strings.ReplaceAll(addr, host.Host, cdn.IP.String()))
			},
	...
	for i := 0; i < config.Test.HTTPCount; i++ {
		for _, test := range host.Test {
			...
			select {
			case <-done:
			case <-time.After(config.HTTP.TimeoutIdle.Duration):
				hres.Body.Close()
			}

			secs := time.Now().Sub(start).Seconds()

			if (err == nil || err == io.EOF) && sz > 0 && hex.EncodeToString(h.Sum(nil)) == test.SHA1 {
				cdn.HTTP.Response++
